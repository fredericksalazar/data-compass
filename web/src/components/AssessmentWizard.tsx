  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white dark:bg-slate-950 gap-4">
        <div className="h-8 w-8 rounded-full border-2 border-blue-600 dark:border-blue-500 border-t-transparent animate-spin" />
        <p className="text-slate-500 dark:text-slate-400 text-sm">Cargando evaluación...</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-slate-950 px-4">
        <div className="rounded-xl border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/10 p-8 max-w-md text-center">
          <p className="text-red-600 dark:text-red-400 font-medium mb-4">No se pudo conectar con el servidor</p>
      ...
      <ResultDashboard
        ...
        donationSlot={
          <div className="text-center">
            ...
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">¿Te ha servido este diagnóstico?</h3>
            ...
            <p className="mt-4 text-xs text-slate-400 dark:text-slate-600">Donación mínima sugerida: $10 USD · 100% voluntario</p>
          </div>
        }
      />
    );
    ...
    <>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-white/80 dark:bg-slate-950/90 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-2xl border border-slate-200 dark:border-slate-700/60 bg-white dark:bg-slate-900 shadow-2xl">
              ...
              <div className="px-8 py-10 text-center">
                <div className="flex items-center justify-center mb-6">
                  <div className="relative h-16 w-16">
                    ...
                    <div className="absolute inset-0 rounded-full border-2 border-t-blue-500 dark:border-t-blue-400 animate-spin" />
                ...
                <h3 className="text-lg font-semibold text-slate-900 dark:text-white mb-1">Procesando tu diagnóstico</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mb-8 h-5 transition-all duration-500">
                ...
                <p className="text-xs text-slate-400 dark:text-slate-600">{Math.round(barProgress)}% completado</p>
              </div>
            ) : showLeadForm ? (
              <LeadFormModal
                apiUrl={API_URL}
        ...
        <nav className="flex items-center justify-between px-6 lg:px-8 h-16 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-slate-500 dark:text-slate-400">
              Pregunta <span className="text-slate-900 dark:text-white font-medium">{currentIndex + 1}</span> de {allQuestions.length}
            </span>
          </div>
        </nav>
          ...
          <div className="w-full max-w-2xl">
            {currentDomainName && (
              <div className="flex items-center gap-2 mb-6">
                <span className="inline-flex items-center rounded-full border border-blue-500/30 bg-blue-50 dark:bg-blue-500/10 px-3 py-1 text-xs font-medium text-blue-700 dark:text-blue-300">
                  {currentDomainName}
                </span>
                <span className="text-xs text-slate-400 dark:text-slate-600">{Math.round(progress)}% completado</span>
              </div>
            )}

            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm dark:shadow-xl p-8 mb-4">
              <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-200 mb-2">{currentQuestion.title}</h2>
              <p className="text-slate-500 dark:text-slate-400 leading-relaxed mb-8">{currentQuestion.question_text}</p>
                  ...
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-lg border-2 transition-all duration-150 group ${
                      selectedLevel === option.level
                        ...
                        : 'border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 hover:border-blue-500/50 hover:bg-blue-50/50 dark:hover:bg-slate-800'
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      <span className={`flex-shrink-0 h-5 w-5 rounded-full border-2 mt-0.5 flex items-center justify-center transition-colors ${
                        selectedLevel === option.level
                          ...
                          : 'border-slate-300 dark:border-slate-600 group-hover:border-blue-500/50'
                      }`}>
                        {selectedLevel === option.level && (
                      ...
                      <span className={`text-sm leading-relaxed ${selectedLevel === option.level ? 'text-slate-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                        <span className="font-medium text-slate-400 dark:text-slate-500 mr-1">N{option.level}.</span>
                        {option.text}
                      </span>
                    </div>
                  </button>
